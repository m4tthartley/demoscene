...
void main() {
	...
	for (int x = int(-floor(samples/2)); x < int(ceil(samples/2)); ++x) {
		color += texture(rt_tex, (screen_pos*0.5+0.5)+vec2(pix.x*float(x), pix.y*float(y))).rgb;
	}
	...
	for (int stp = 0; stp < samples; ++stp) {
		...
		color += texture(rt_tex, (screen_pos*0.5+0.5)+a).rgb;
	}
	out_color = vec4(color/(samples*samples), 1.0);
#else
	out_color = texture(rt_tex, screen_pos*0.5+0.5);
#endif
}
